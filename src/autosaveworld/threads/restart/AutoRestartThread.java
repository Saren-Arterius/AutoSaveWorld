...
public class AutoRestartThread extends Thread {
    ...
    public void run() {
        ...
        // check if we just restarted (server can restart faster than 1 minute.
        // Without this check AutoRestartThread will stop working after restart)
        ...
        while (run) {
            if ((config.autorestart && config.autorestarttime.contains(getCurTime())) || command) {
                ...
                if (config.autorestartcountdown && !skipcountdown) {
                    for (int i = config.autorestartbroadcastonseconds.get(0); i > 0; i--) {
                        if (config.autorestartbroadcastonseconds.contains(i)) {
                            MessageLogger
                                    .broadcast(
                                            configmsg.messageAutoRestartCountdown.replace("{SECONDS}",
                ...
                SchedulerUtils.callSyncTaskAndWait(new Runnable() {
                    @Override
                    public void run() {
                        for (final String command: config.autorestartcommmands) {
    ...
    private String getCurTime() {
        final String curtime = sdf.format(System.currentTimeMillis());
        return curtime;
    }
