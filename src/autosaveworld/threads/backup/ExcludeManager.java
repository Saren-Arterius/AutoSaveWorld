...
public class ExcludeManager {
    ...
    public static boolean isFolderExcluded(List<String> excludelist, String folderPath) {
        ...
        for (final String excludedFolderPath: excludelist) {
            if (excludedFolderPath.contains("*")) {
                // resolve wildcard
                // check parents folders equality
                if (folder.getAbsoluteFile().getParentFile()
                        .equals(new File(excludedFolderPath).getAbsoluteFile().getParentFile())) {
                    // check name equality
                    final String excludeFolderName = new File(excludedFolderPath).getName();
        ...
        if (Bukkit.getWorlds().get(0).getWorldFolder().getAbsoluteFile().equals(folder.getParentFile())) {
            // if this folder name is equal to one of the world names we should
            // ignore it
