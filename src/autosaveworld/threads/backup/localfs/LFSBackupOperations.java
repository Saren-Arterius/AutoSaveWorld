...
public class LFSBackupOperations {
    ...
    public void startWorldBackup(ExecutorService backupService, final World world, final int maxBackupsCount,
            final String latestbackuptimestamp) {
        // create runnable
        final Runnable backupWorld = new Runnable() {
            @Override
            public void run() {
                MessageLogger.debug("Starting backup for world " + world.getWorldFolder().getName());
                final String worldbackupfolder = extpath + File.separator + "backups" + File.separator + "worlds"
                        + File.separator + world.getWorldFolder().getName();
                // check oldest backup count
                if (maxBackupsCount != 0 && new File(worldbackupfolder).exists()
                        && new File(worldbackupfolder).list().length >= maxBackupsCount) {
                    // remove oldest backup
                    MessageLogger.debug("Deleting oldest backup for world " + world.getWorldFolder().getName());
                    // find oldest backup
                    final String oldestBackupName = BackupFileUtils.findOldestBackupNameLFS(worldbackupfolder);
                    // delete oldest backup
                    if (oldestBackupName != null) {
                        final File oldestBakup = new File(worldbackupfolder + File.separator + oldestBackupName);
                        FileUtils.deleteDirectory(oldestBakup);
                    }
                }
                MessageLogger.debug("Backuping world " + world.getWorldFolder().getName());
                final boolean savestatus = world.isAutoSave();
                world.setAutoSave(false);
                final File worldfolder = world.getWorldFolder().getAbsoluteFile();
                final String worldBackup = worldbackupfolder + File.separator + latestbackuptimestamp;
                if (!zip) {
    ...
    public void startPluginsBackup(ExecutorService backupService, final int maxBackupsCount,
            final String latestbackuptimestamp) {
        // create runnable
        final Runnable backupPlugins = new Runnable() {
            @Override
            public void run() {
                ...
                // check oldest backup count
                if (maxBackupsCount != 0 && new File(pluginsbackupfolder).exists()
                        && new File(pluginsbackupfolder).list().length >= maxBackupsCount) {
                    // remove oldest backup
                    MessageLogger.debug("Deleting oldest plugins backup");
                    // find oldest backup
                    final String oldestBackupName = BackupFileUtils.findOldestBackupNameLFS(pluginsbackupfolder);
                    // delete oldest backup
                    if (oldestBackupName != null) {
                        final File oldestBakup = new File(pluginsbackupfolder + File.separator + oldestBackupName);
                        FileUtils.deleteDirectory(oldestBakup);
                    }
                }
                MessageLogger.debug("Backuping plugins");
                final File pluginsfolder = plugin.getDataFolder().getParentFile().getAbsoluteFile();
                final String pluginsBackup = extpath + File.separator + "backups" + File.separator + "plugins"
                        + File.separator + latestbackuptimestamp;
                if (!zip) {
    ...
    public void deleteOldestPluginsBackup(String oldestbackupdate) {
        final String fldtodel = extpath + File.separator + "backups" + File.separator + "plugins" + File.separator
                + oldestbackupdate;
