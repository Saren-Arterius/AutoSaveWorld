...
public class FTPBackupOperations {
    ...
    public void backupWorld(World world) {
        ...
        final boolean savestaus = world.isAutoSave();
        world.setAutoSave(false);
        try {
            final File worldfolder = world.getWorldFolder().getCanonicalFile();
            if (!zip) {
    ...
    public void backupPlugins() {
        try {
            final File plfolder = plugin.getDataFolder().getParentFile().getCanonicalFile();
            if (!zip) {
