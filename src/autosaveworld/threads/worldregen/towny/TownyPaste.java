...
public class TownyPaste {
    ...
    public void pasteAllFromSchematics() {
        try {
            ...
            final String schemfolder = GlobalConstants.getTownyTempFolder();

            final List<Town> towns = TownyUniverse.getDataSource().getWorld(wtopaste.getName()).getTowns();
                ...
                if (tblocks.size() > 0) {
                    ...
                    for (final TownBlock tb: tblocks) {
                        if (tb.getWorld().getName().equals(wtopaste.getName())) {
                            ...
                            final SchematicToLoad schematicdata = new SchematicToLoad(schemfolder + town.getName()
