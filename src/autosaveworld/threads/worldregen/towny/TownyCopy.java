...
public class TownyCopy {
    ...
    public void copyAllToSchematics() {
        try {
            ...
            final List<Town> towns = TownyUniverse.getDataSource().getWorld(wtoregen.getName()).getTowns();
            for (final Town town: towns) {
                final List<TownBlock> tblocks = town.getTownBlocks();
                if (tblocks.size() > 0) {
                    ...
                    for (final TownBlock tb: tblocks) {
                        if (tb.getWorld().getName().equals(wtoregen.getName())) {
                            // get coords
                            final int xcoord = tb.getX();
                            final int zcoord = tb.getZ();
                            final Vector bvmin = BukkitUtil
                                    .toVector(new Location(wtoregen, xcoord * 16, 0, zcoord * 16));
                            ...
                            final SchematicToSave schematicdata = new SchematicToSave(
                                    GlobalConstants.getTownyTempFolder() + town.getName() + File.separator + "X"
                                            + xcoord + "Z" + zcoord, bvmin, bvmax);
