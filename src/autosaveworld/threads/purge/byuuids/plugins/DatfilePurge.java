...
public class DatfilePurge {
    ...
    public void doDelPlayerDatFileTask(ActivePlayersList pacheck) {
        ...
        final File playersdatfolder = new File(worldfoldername + File.separator + "playerdata" + File.separator);
        for (final File playerfile: playersdatfolder.listFiles()) {
            if (playerfile.getName().endsWith(".dat")) {
                final String playeruuid = playerfile.getName().substring(0, playerfile.getName().length() - 4);
