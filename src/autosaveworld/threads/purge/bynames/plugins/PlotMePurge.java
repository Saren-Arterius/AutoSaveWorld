    ...
    public void doPlotMePurgeTask(ActivePlayersList pacheck, final boolean regenplot) {
        ...
        for (final World w: Bukkit.getWorlds()) {
            if (PlotManager.isPlotWorld(w)) {
                ...
                final HashSet<Plot> plots = new HashSet<Plot>(PlotManager.getPlots(w).values());
                for (final Plot p: plots) {
                    ...
                    if (!pacheck.isActiveCS(p.getOwner())) {
                        ...
                        final Runnable delPlot = new Runnable() {
                            @Override
                            public void run() {
                                if (regenplot) {
                                ...
                                SqlManager.deletePlot(PlotManager.getIdX(PlotID), PlotManager.getIdZ(PlotID), w
                                        .getName().toLowerCase());
                            }
                        };
