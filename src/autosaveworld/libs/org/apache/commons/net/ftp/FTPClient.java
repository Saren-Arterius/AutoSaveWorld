 ...
 * {@link autosaveworld.libs.org.apache.commons.net.SocketClient#disconnect
 ...
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTP#getReplyCode
 ...
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTPReply#SERVICE_NOT_AVAILABLE
 ...
 * an
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTPConnectionClosedException}
 * , you must disconnect the connection with {@link #disconnect disconnect() }
 * to properly clean up the system resources used by FTPClient. Before
 * disconnecting, you may check the last reply code and text with
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTP#getReplyCode
 * getReplyCode },
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTP#getReplyString
 * getReplyString }, and
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTP#getReplyStrings
 * getReplyStrings}. You may avoid server disconnections while the client is
 * idle by periodically sending NOOP commands to the server.
 * <p>
...
public class FTPClient extends FTP {
    ...
    private void __initDefaults() {
        __dataConnectionMode = FTPClient.ACTIVE_LOCAL_DATA_CONNECTION_MODE;
        __passiveHost = null;
        __passivePort = -1;
     ...
     *         or the full string after the reply code and space if the syntax
     *         is invalid
     *         (i.e. enclosing quotes are missing or embedded quotes are not
     *         doubled)
     */
    // package protected for access by test cases
    static String __parsePathname(String reply) {
        final String param = reply.substring(FTP.REPLY_CODE_LEN + 1);
        if (param.startsWith("\"")) {
    ...
    protected void _parsePassiveModeReply(String reply) throws MalformedServerReplyException {
        final java.util.regex.Matcher m = FTPClient.__PARMS_PAT.matcher(reply);
        if (!m.find()) {
        ...
        if (__passiveNatWorkaround) {
            try {
                ...
                if (host.isSiteLocalAddress()) {
                    final InetAddress remote = getRemoteAddress();
                    if (!remote.isSiteLocalAddress()) {
                        final String hostAddress = remote.getHostAddress();
                        fireReplyReceived(0, "[Replacing site local address " + __passiveHost + " with " + hostAddress
                                + "]\n");
                        __passiveHost = hostAddress;
                    }
                }
            } catch (final UnknownHostException e) { // Should not happen as we
                                                     // are passing in an IP
                                                     // address
                throw new MalformedServerReplyException("Could not parse passive host information.\nServer Reply: "
                        + reply);
            }
    ...
    protected boolean _storeFile(String command, String remote, InputStream local) throws IOException {
        ...
        final OutputStream output = getBufferedOutputStream(socket.getOutputStream());
    ...
    protected Socket _openDataConnection_(String command, String arg) throws IOException {
        if (__dataConnectionMode != FTPClient.ACTIVE_LOCAL_DATA_CONNECTION_MODE
                && __dataConnectionMode != FTPClient.PASSIVE_LOCAL_DATA_CONNECTION_MODE) {
            return null;
        }
     ...
     * must remember to call {@link #enterLocalActiveMode enterLocalActiveMode()
     * } if you
    ...
    public boolean enterRemoteActiveMode(InetAddress host, int port) throws IOException {
        if (FTPReply.isPositiveCompletion(port(host, port))) {
            __dataConnectionMode = FTPClient.ACTIVE_REMOTE_DATA_CONNECTION_MODE;
            __passiveHost = null;
            __passivePort = -1;
            return true;
        }
    ...
    public boolean enterRemotePassiveMode() throws IOException {
        ...
        __dataConnectionMode = FTPClient.PASSIVE_REMOTE_DATA_CONNECTION_MODE;
        _parsePassiveModeReply(_replyLines.get(0));
     ...
     * it again. The default file type is <code> FTP.ASCII_FILE_TYPE </code> if
    ...
    public boolean remoteRetrieve(String filename) throws IOException {
        if (__dataConnectionMode == FTPClient.ACTIVE_REMOTE_DATA_CONNECTION_MODE
                || __dataConnectionMode == FTPClient.PASSIVE_REMOTE_DATA_CONNECTION_MODE) {
     ...
     *                transferring the file. The CopyStreamException allows you
     *                to
     *                determine the number of bytes transferred and the
     *                IOException
    ...
    protected boolean _retrieveFile(String command, String remote, OutputStream local) throws IOException {
        ...
        final InputStream input = getBufferedInputStream(socket.getInputStream());
     ...
     * of server-supported exensions.
     * For example, a typical client-server interaction might be (from RFC
     * 2389):
     * 
     * <pre>
     *         C> feat
     ...
     *         array if none)
     *         Returns {@code null} if the feature is not found or the command
     *         failed.
     ...
     *         or the {@link #feat()} command failed. Check
     ...
     *         call {@link #getReplyCode()} or {@link #getReplyString()} to get
     *         the reason.
     * 
     * @exception IOException
     *                If an I/O error occurs while either sending a
    ...
    public String printWorkingDirectory() throws IOException {
        ...
        return FTPClient.__parsePathname(_replyLines.get(_replyLines.size() - 1));
    }
     ...
     * If the value is positive, the option will be set when the data socket has
     * been created.
     * 
     * @param bufSize
     ...
     * It depends on your ftp server if you need to call this method, also dont
     * expect to get rid
