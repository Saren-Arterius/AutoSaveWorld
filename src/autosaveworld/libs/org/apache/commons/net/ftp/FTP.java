 ...
 * have to use the FTP class. The
 * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTPClient} class,
 * derived from FTP, implements all the functionality required of an FTP client.
 * The FTP class is made public to provide access to various FTP constants and
 * to make it easier for adventurous programmers (or those with special needs)
 * to interact with the FTP protocol and implement their own clients. A set of
 * methods with names corresponding to the FTP command names are provided to
 ...
 * to properly clean up the system resources used by FTP. Before disconnecting,
 * you may check the last reply code and text with {@link #getReplyCode
 * getReplyCode }, {@link #getReplyString getReplyString }, and
 * {@link #getReplyStrings getReplyStrings}. You may avoid server disconnections
 * while the client is idle by periodicaly sending NOOP commands to the server.
 * <p>
...
public class FTP extends SocketClient {
     ...
     * EBCDIC formats. All constants ending in <code>FILE_TYPE</code> are used
     ...
     * of records. All constants ending in <code>_STRUCTURE</code> are used to
     ...
     * compressed data. All constants ending in <code>TRANSFER_MODE</code> are
     * used to indicate file transfer modes.
     ***/
    ...
    private boolean __lenientCheck(String line) {
        return (!(line.length() > FTP.REPLY_CODE_LEN && line.charAt(FTP.REPLY_CODE_LEN) != '-' && Character
    ...
    private void __getReply(boolean reportReply) throws IOException {
        ...
        // to be thrown.
        length = line.length();
        if (length < FTP.REPLY_CODE_LEN) {
            throw new MalformedServerReplyException("Truncated server reply: " + line);
        }

        String code = null;
        try {
            code = line.substring(0, FTP.REPLY_CODE_LEN);
        ...
        if (length > FTP.REPLY_CODE_LEN && line.charAt(FTP.REPLY_CODE_LEN) == '-') {
            do {
    ...
    public int type(int fileType) throws IOException {
        return sendCommand(FTPCmd.TYPE, FTP.__modes.substring(fileType, fileType + 1));
    }
    ...
    public int stru(int structure) throws IOException {
        return sendCommand(FTPCmd.STRU, FTP.__modes.substring(structure, structure + 1));
    }
    ...
    public int mode(int mode) throws IOException {
        return sendCommand(FTPCmd.MODE, FTP.__modes.substring(mode, mode + 1));
    }
     ...
     * level of access, use
     * {@link autosaveworld.libs.org.apache.commons.net.ftp.FTPClient} , which
     * will handle all low level details for you.
     * <p>
     * 
     * @param pathname
     ...
     *            may be {@code null} in which case the command is sent with no
     *            parameters
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
