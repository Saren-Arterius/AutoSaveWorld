...
public class SchedulerUtils {
    ...
    public static void callSyncTaskAndWait(Runnable run) {
        SchedulerUtils.scheduleSyncTaskAndWaitInternal(run, 0);
    }

    public static void callSyncTaskAndWait(Runnable run, int timeout) {
        SchedulerUtils.scheduleSyncTaskAndWaitInternal(run, timeout);
    }
    ...
    private static void scheduleSyncTaskAndWaitInternal(final Runnable run, int timeout) {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(SchedulerUtils.plugin, new Runnable() {
            @Override
            public void run() {
                run.run();
                lock.offer(true);
    ...
    public static void scheduleSyncTask(Runnable run) {
        Bukkit.getScheduler().scheduleSyncDelayedTask(SchedulerUtils.plugin, run);
    }

    public static void scheduleSyncRepeatingTask(Runnable run, int delay, int interval) {
        Bukkit.getScheduler().scheduleSyncRepeatingTask(SchedulerUtils.plugin, run, delay, interval);
    }
